...
uint64_t accept4_addr = 0;
uint64_t accept4_addrlen = 0;
uint64_t getsockname_addr = 0;
uint64_t getsockname_addrlen = 0;
...
void linux_syscalls::print_syscall(drakvuf_t drakvuf, drakvuf_trap_info_t* info, std::vector<uint64_t> arguments)
{
    ...
    //下面是我的
    bool check_syscall_name = false;
    bool check_syscall_name_accept = false;
    bool check_syscall_name_getsockname = false;
//    printf("syscall name is: %s\n",params->sc->display_name);
    if (strcmp(params->sc->display_name, "bind") == 0) check_syscall_name= true;
    if (strcmp(params->sc->display_name, "connect") == 0) check_syscall_name= true;
    if (strcmp(params->sc->display_name, "accept") == 0) check_syscall_name_accept= true;
    if (strcmp(params->sc->display_name, "accept4") == 0) check_syscall_name_accept= true;
    if (strcmp(params->sc->display_name, "getsockname") == 0) check_syscall_name_getsockname = true;

//    else printf("no\n");
    uint64_t test_value[2] = {0};
    ...
    for (size_t i = 0; i < arguments.size(); i++)
    {
        ...
        else
        {
            ...
            //下面是我改的
            if (check_syscall_name && (strcmp(params->sc->args[i].name, "addr") == 0)){
//                printf("get addr!!!\n");
                test_value[0] = value;
            }else if (check_syscall_name && (strcmp(params->sc->args[i].name, "addrlen") == 0)){
//                printf("get addrlen!!\n");
                test_value[1] = value;
                if(test_value[1]!=0x10) check_syscall_name= false;
            }else if (check_syscall_name_accept && (strcmp(params->sc->args[i].name, "addr") == 0)){
                test_value[0] = value;

            }else if (check_syscall_name_accept && (strcmp(params->sc->args[i].name, "addrlen") == 0)){
                test_value[1] = value;
            }else if (check_syscall_name_getsockname && (strcmp(params->sc->args[i].name, "addr") == 0)){
                test_value[0] = value;
            }else if (check_syscall_name_getsockname && (strcmp(params->sc->args[i].name, "addrlen") == 0)){
                test_value[1] = value;
            }
    ...
    //下面是我改的
    if(check_syscall_name){
//        printf("bind/connect addr is: %" PRIu64 "\t",test_value[0]);
//        printf("the len is: %lu\n",test_value[1]);
        uint64_t output = this->test_read_value(drakvuf, info, test_value[0], test_value[1]);
//        printf("addrval is: %" PRIu64 "\n", output);
        this->fmt_args.push_back(keyval("readval",fmt::Nval(output)));
    }else if (check_syscall_name_accept){
        uint64_t output_addrlen = this->test_read_value_addrlen(drakvuf, info, test_value[1]);
//        printf("accept addrlen is: %lu\n",output_addrlen);
//        printf("addr: %" PRIu64 "\n",test_value[0]);
//        uint64_t output = this->test_read_value(drakvuf, info, test_value[0], 0x10);
//        printf("the value is: %" PRIu64 "\n",output);
        accept4_addr = test_value[0];
        accept4_addrlen = output_addrlen;
        this->fmt_args.push_back(keyval("lenval",fmt::Nval(output_addrlen)));
    }else if(check_syscall_name_getsockname){
        uint64_t output_addrlen = this->test_read_value_addrlen(drakvuf, info, test_value[1]);

    }


    check_syscall_name = false;
    check_syscall_name_accept = false;
...
event_response_t linux_syscalls::linux_ret_cb(drakvuf_t drakvuf, drakvuf_trap_info_t* info)
{
    ...
    bool check_syscall_name_accept = false;
    bool check_itvp4 = false;
    if (accept4_addrlen==16) check_itvp4 = true;
    if ((strcmp(info->trap->name, "accept4") == 0)||(strcmp(info->trap->name, "accept") == 0)){
        check_syscall_name_accept = true;
    }
//    printf("display_name is :%s\n",info->trap->name);
    if ((check_syscall_name_accept)&& (accept4_addr != 0)&& check_itvp4) {
        uint64_t output = this->test_read_value(drakvuf, info, accept4_addr, 0x10);
//        printf("addrvalue is: %" PRIu64 "\n",output);

        this->fmt_args.clear();
        this->fmt_args.push_back(keyval("readfrom", fmt::Xval(accept4_addr)));
        this->fmt_args.push_back(keyval("addrlen", fmt::Nval(accept4_addrlen)));
        this->fmt_args.push_back(keyval("readval", fmt::Nval(output)));
        this->print_sysret_accept(drakvuf, info, (uint64_t) params->num, &(this->fmt_args));
//        printf("check is ok!!!!!\n");
