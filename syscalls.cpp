...
void syscalls_base::print_sysret_accept(drakvuf_t drakvuf, drakvuf_trap_info_t* info, int nr, const std::vector<std::pair<char const*, fmt::Aarg>>* fmt_args)
{
    fmt::print(this->m_output_format, "sysret", drakvuf, info,
               keyval("Module", fmt::Qstr(std::move(info->trap->breakpoint.module))),
               keyval("vCPU", fmt::Nval(info->vcpu)),
...
//下面是我改的，也在类中添加了这个函数声明
uint64_t syscalls_base::test_read_value(drakvuf_t drakvuf, drakvuf_trap_info_t* info, uint64_t addr, uint64_t addrlen){
    ...
    uint64_t _val;
    if (VMI_FAILURE == vmi_read(vmi, &ctx, addrlen, &_val, nullptr))
    {
        fprintf(stderr, "in test_read_value Failed to read address (%p)\n", (void*) addr);
        _val = 0;
    }
    return _val;
}
uint32_t  syscalls_base::test_read_value_addrlen(drakvuf_t drakvuf, drakvuf_trap_info_t *info, uint64_t addr) {
    auto  vmi = vmi_lock_guard(drakvuf);
    ACCESS_CONTEXT(ctx,
            .translate_mechanism = VMI_TM_PROCESS_DTB,
    .dtb = info->regs->cr3,
    ...
    uint32_t _val;
    if (VMI_FAILURE == vmi_read_32(vmi, &ctx, &_val))
    {
        fprintf(stderr, "in test_read_value_addrlen Failed to read address (%p)\n", (void*) addr);
        _val = 0;
    }
    return _val;
}
