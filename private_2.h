...
public:
    ...
    //下面是我加的
    uint64_t test_read_value(drakvuf_t drakvuf, drakvuf_trap_info_t* info, uint64_t addr, uint64_t addrlen);
    uint32_t test_read_value_addrlen(drakvuf_t drakvuf, drakvuf_trap_info_t* info, uint64_t addr);
    void print_sysret_accept(drakvuf_t drakvuf, drakvuf_trap_info_t* info, int nr, const std::vector<std::pair<char const*, fmt::Aarg>>* fmt_args);
